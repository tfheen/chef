...
describe Chef::Runner do
  ...
  it "should send a resources only_if to Chef::Mixin::Command.only_if" do
    ...
    Chef::Mixin::Command.should_receive(:only_if).with(true, nil).and_return(false)
    @runner.converge
  end
  ...
  it "should change to the directory specified in cwd for only_if" do
    ...
    @collection[0].should_receive(:cwd).and_return("/tmp")
    Chef::Mixin::Command.should_receive(:only_if).with("/bin/true", "/tmp").and_return(true)
    @runner.converge
  end
  ...
  it "should send a resources not_if to Chef::Mixin::Command.not_if" do
    ...
    Chef::Mixin::Command.should_receive(:not_if).with(true, nil).and_return(false)
    @runner.converge
  end
  ...
  it "should change to the directory specified in cwd for not_if" do
    ...
    @collection[0].should_receive(:cwd).and_return("/tmp")
    Chef::Mixin::Command.should_receive(:not_if).with("/bin/true", "/tmp").and_return(true)
    @runner.converge
  end
